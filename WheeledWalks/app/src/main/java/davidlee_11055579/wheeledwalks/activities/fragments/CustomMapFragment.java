...
public class CustomMapFragment extends com.google.android.gms.maps.SupportMapFragment {
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mObstacleCount = 0;
        mMap = getMap();
        mMap.setMapType(MAP_TYPE_TERRAIN);  //topo map type
     ...
     * for different colours for each polyline segment (coming in V1.2)
     */
    private void plotRoute() {

        //TODO: colour change based on steepness for v1.2
        if (mLastLocation != null) {

            PolylineOptions track = new PolylineOptions();
            track.add(mLastLocation);
            track.add(mMyLocation);
            //draw the line and add it to the path list
            mWalkPathList.add(mMap.addPolyline(track.color(Color.RED).width(5)));

            mLastLocation = mMyLocation;
        }
    }
